...
\mainmatter
\part{Constantine Caramanis 的课程}

\chapter{基本定义}
\section{基本的线性代数和微积分}
\section{凸集合和凸函数}
\begin{introduction}
  \item 凸集合~\ref{convexset}
  \item 字空间和放射空间~\ref{subspace}
  \item 半空间/超平面~\ref{halfspace}
  \item 锥~\ref{cone}
  \item 凸函数的四种定义 ~\ref{con-func}
\end{introduction}
\subsection{凸集合} \label{convexset}
\begin{definition}{凸集合}
r如果$x_1, x_2 \in \mathbb{C}$ 凸集合，那么$\exists 0 \leq \lambda \leq 1$使得$x_1 + \lambda ( x_2 - x_1 ) \in \mathbb{C}$.
\end{definition}
\subsection{子空间和仿射空间} \label{subspace}
\begin{definition}{span space}
r列向量$v_1,v_2, \cdots, vk$，由其张成的空间为$V = \text{span} \{ v_1,v_2, \cdots, vk \} = \{  v = \sum_{i=1}^{k} \alpha_i v_i, \ \alpha_i \in \mathbb{R}  \}$.记矩阵$M=[v_1,v_2, \cdots, vk]$，则$V = \text{Range} (M)$.
\end{definition}


\begin{definition}{null space}
r $\text{Nullspace} (M) = \{ x: Mx = 0 \}$.
\end{definition}

对于直线$y = ax + b$，整理为$-ax + y = b$，设$ s = [ -a, 1 ]^T, X = [ x , y  ]^T $，则直线可以表达为$ s^T X = b $。对于多维度的，可类似如此表达。
\subsection{半空间/超平面} \label{halfspace}
\subsection{锥} \label{cone}
\subsection{凸函数} \label{con-func}


\section{凸优化的最优性条件}
\begin{introduction}
  \item 无约束~\ref{unconstraint}
  \item 有约束~\ref{constraint}
  \item 投影~\ref{projection}
\end{introduction}

\subsection{无约束问题一阶最优条件} \label{unconstraint}
\subsection{有约束问题一阶最优条件} \label{constraint}
\subsection{投影} \label{projection}
...
\section{梯度和次梯度下降}
\begin{introduction}
    \item 梯度下降~\ref{gradient-descent}
    \item 次梯度下降~\ref{subgradient-descent}
    \item 三性质~\ref{triple}
\end{introduction}
\subsection{梯度下降} \label{gradient-descent}
\subsection{次梯度下降} \label{subgradient-descent}
\subsection{抽象的三条性质} \label{triple}
\section{光滑和强凸性质}
\begin{introduction}
    \item 光滑~\ref{smooth}
    \item 强凸~\ref{strong}
    \item 光滑凸函数~\ref{smoo-func}
    \item 强凸函数~\ref{strong-func}
\end{introduction}
\subsection{光滑} \label{smooth}
\subsection{强凸} \label{strong}
\subsection{光滑凸函数} \label{smoo-func}
\subsection{强凸函数} \label{strong-func}
\section{收敛速度}
\section{加速/动量梯度}
\section{近端和投影梯度}
\subsection{投影梯度}
